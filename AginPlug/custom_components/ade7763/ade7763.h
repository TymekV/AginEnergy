...
class ADE7763Sensor : public PollingComponent, public spi::SPIDevice<spi::BIT_ORDER_MSB_FIRST, spi::CLOCK_POLARITY_LOW, spi::CLOCK_PHASE_TRAILING, spi::DATA_RATE_1MHZ> {
public:
	void setup() override;
	...
	uint32_t ADE_read (unsigned char addr,unsigned char count);
	void ADE_write(unsigned char addr,uint32_t data, unsigned char count);

	/* https://github.com/whatnick/ADE7763_Arduino/blob/master/ADE7763.ino */
	void resetPeaks(void);
	void setInterrupt(unsigned int interrupt);




	void set_reset_pin(GPIOPin *reset) { reset_pin_ = reset; }
	void set_voltage_sensor(sensor::Sensor  *v) { voltage_sensor_ = v; }
	void set_current_sensor(sensor::Sensor  *v) { current_sensor_ = v; }
	void set_power_sensor(sensor::Sensor  *v)   { power_sensor_ = v; }
