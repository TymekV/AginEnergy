...
namespace esphome
{
	namespace ade7763
	{
		...
		void ADE7763Sensor::update()
		{
			...
			if (power_sensor_)
			{
				...
				{
					power_sensor_->publish_state(last_r);
				}
			}
	...
	if (this->has_fault_()) {
		...
		uint32_t ADE7763Sensor::ADE_read(unsigned char addr, unsigned char count)
		{
			...
			case 2:
			{
				ret = (((uint32_t)this->read_byte()) << 8);
				ret |= ((uint32_t)this->read_byte());
			}
			case 3:
			{
				ret = (((uint32_t)this->read_byte()) << 16);
				ret |= (((uint32_t)this->read_byte()) << 8);
				ret |= ((uint32_t)this->read_byte());
			}
			default:
		...
		void ADE7763Sensor::ADE_write(unsigned char addr, uint32_t data, unsigned char count)
		{
			...
			case 2:
			{
				this->write_byte((data >> 8) & 0xff);
				this->write_byte(data & 0xff);
			}
			break;
			case 3:
			{
				this->write_byte((data >> 16) & 0xff);
				this->write_byte((data >> 8) & 0xff);
				this->write_byte(data & 0xff);
			}
			break;
